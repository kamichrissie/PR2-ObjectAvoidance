...
void LineExtractionROS::populateMarkerMsg(const std::vector<Line> &lines, 
  ...
  for (std::vector<Line>::const_iterator cit = lines.begin(); cit != lines.end(); ++cit)
  {
    ...
    //What width?
   // Ellipse ellipse(testpoint.x,testpoint.y,testpoint.z, r1, r2);
    ...
    //LineExtractionROS::ellipses_coordinates(px,py,alpha_ellipse,r2,r1);
...
double* LineExtractionROS::ellipses_coordinates( double dx, double dy, double alpha, double r1, double r2){
...
double** LineExtractionROS::ellipses_pass(const std::vector<Line> &lines){
